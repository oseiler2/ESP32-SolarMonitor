    ...
    <div class="container">
      <div class="innerDiv">
        ...
        <fieldset>
          ...
          <form action="/update" method="get"><button class="btn">Update firmware</button></form><br>
          <form action="/wifiOff" method="get"><button class="btn">Disable Wifi</button></form>
  ...
  source.addEventListener(
    ...
    function (e) {
      ...
      const matches = d.match(
        /^(?:\033\[\d;\d*m)?\[[ ]*(?<uptime>[\d]+)]\[(?<level>[ DEWVI])\]\[(?<tag>[^\]]*)\]\[(?<file>.+):(?<line>\d+) - (?<method>[^\]]+)\] (?<msg>[^\033]*)(?:\033\[0m)?$/
      );
      const [all, uptime = "", level = "", tag = "", file = "", line = "", method = "", msg = ""] = matches;
      const record = {
        uptime,
        level,
        tag,
...
<style>
  ...
  <body>
    <div class="container">
      <div class="innerDiv">
        <h2>Configuration</h2>
        <form novalidate method='get' action='configsave'>
          <fieldset>
)";
...
<div><label for='{i}'>{p}</label><input id='{i}' name='{n}' type='number' maxlength={l} value='{v}' min='{mi}' max='{ma}' class='{c}'></div>)";

  const char config_parameter_fraction[] PROGMEM = R"(
<div><label for='{i}'>{p}</label><input id='{i}' name='{n}' type='number' maxlength={l} value='{v}' min='{mi}' max='{ma}' step='any' class='{c}'></div>)";
...
<option value='{v}' {s}>{lbl}</option>)";
  ...
  const char config_footer[] PROGMEM = R"(
    ...
    <script>
      ...
      function togglePreset(usePreset) {
        document.getElementById("bw").closest("div").style.display = usePreset ? 'none' : '';
        document.getElementById("sf").closest("div").style.display = usePreset ? 'none' : '';
        document.getElementById("cr").closest("div").style.display = usePreset ? 'none' : '';
      }

      document.getElementById("usePst").onchange = async(evt) => {
        togglePreset(evt.target.checked);
         Array.from(document.forms[0].elements).forEach((input) => {
          if (input.tagName === "INPUT" || input.tagName === "SELECT") validateInput(input);
        });
      };
      ...
      function validateInput(input) {
        ...
        if (input.closest("div").style.display === 'none') {
          ...
          if (input.tagName === "INPUT" && input.classList.contains("zeroAble")) {
            if (0 == input.value) {
              return true;
            } else {
      ...
      document.addEventListener("DOMContentLoaded", async () => {
        document.getElementById("bw").classList.remove("zeroAble");
        document.getElementById("sf").classList.remove("zeroAble");
        document.getElementById("cr").classList.remove("zeroAble");
        togglePreset(document.getElementById("usePreset").checked);
        ...
        document.forms[0].addEventListener("submit", (evt) => {
          let inputValid = true;
          Array.from(evt.target.elements).forEach((input) => {
            if (input.tagName === "INPUT" || input.tagName === "SELECT") {
              inputValid &= validateInput(input);
            }
          }); 
          if (!inputValid) evt.preventDefault();
        });
      });
    </script>    
  </body>
