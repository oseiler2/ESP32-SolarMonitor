...
#define MAX_PW_LEN 64
  ...
  const uint8_t X_CMD_REBOOT = bit(4);
  ...
  void handleWifiOff(AsyncWebServerRequest* request);
  ...
  String getMac() {
    uint8_t mac[8];
    _ASSERT(esp_efuse_mac_get_default(mac) == ESP_OK);
  ...
  */
  ...
  void setupWifiManager(const char* _appName, std::vector<ConfigParameterBase<Config>*> _configParameterVector, bool _keepCaptivePortalActive, bool _captivePortalActiveWhenNotConnected) {
    ...
    server.on("/wifiOff", HTTP_GET, handleWifiOff);
    ...
    if (strlen(PORTAL_USER) == 0 || strlen(PORTAL_PW) == 0) {
      ElegantOTA.begin(&server);
    } else {
      ElegantOTA.begin(&server, PORTAL_USER, PORTAL_PW);
    }
  ...
  void handleWifiOff(AsyncWebServerRequest* request) {
    ESP_LOGI(TAG, "handleWifiOff");
    if (!authenticate(request)) return;
    config.wifiEnabled = false;
    AsyncWebServerResponse* response = request->beginResponse(302, FPSTR(html::content_type_plain), FPSTR(html::reboot));
    response->addHeader("Location", "/");
    response->addHeader(FPSTR(html::header_cache_control), FPSTR(html::cache_control_no_cache));
    request->send(response);
    startTimer("SaveConfig", 1000, saveConfig);
    startTimer("Reboot", 2000, reboot);
  }

  IRAM_ATTR void logCallback(int level, const char* tag, const char* message) {
    // TODO: send to queue?
  ...
  void packetReceivedEvent(const char* msg) {
    // TODO: store in Queue for later processing
//    events.send(msg, "msh", millis());
  }
  ...
  void handleConfig(AsyncWebServerRequest* request) {
    ...
    for (ConfigParameterBase<Config>* configParameter : configParameterVector) {
      ...
      if (configParameter->isNumber()) {
        if (configParameter->isFraction()) {
          parameterHtml = FPSTR(html::config_parameter_fraction);
        } else {
        ...
        char value[configParameter->getMaxStrLen() + 1];
        configParameter->print(config, value);
        parameterHtml.replace("{v}", value);
        parameterHtml.replace("{c}", configParameter->canBeZero() ? "zeroAble" : "");
  ...
  void handleSafeConfig(AsyncWebServerRequest* request) {
    ...
    int8_t res;
    uint8_t changedMask = 0x00;
    for (ConfigParameterBase<Config>* configParameter : configParameterVector) {
      res = (configParameter->save(config, request->arg(configParameter->getId()).c_str()));
    ...
    if (rebootRequired) changedMask = changedMask | CONFIG_REBOOT_REQUIRED;
    ...
    if (changedMask & CONFIG_UPDATED_SAVE_CONFIG) {
      ESP_LOGI(TAG, "Saving config in 1 secs...");
      startTimer("SaveConfig", 1000, saveConfig);
    }
  ...
  void handleNotFound(AsyncWebServerRequest* request) {
    ...
    AsyncWebServerResponse* response = request->beginResponse(302, FPSTR(html::content_type_plain), "");
    response->addHeader("Location", "/");
    response->addHeader(FPSTR(html::header_cache_control), FPSTR(html::cache_control_no_cache));
    request->send(response);
  }
  ...
  void scanWiFi(bool async) {
    ...
    if (!async) {
      // wait until current scan is done
      while (wifiScanActive && (millis() - now < WIFI_SCAN_TIMEOUT)) vTaskDelay(pdMS_TO_TICKS(20));
    }
  }
...
#define reason2str(r) ((r>176)?system_event_reasons[r-176]:system_event_reasons[r-1])
  ...
  */
  ...
  void handleReboot(AsyncWebServerRequest* request) {
    ...
    response->addHeader(FPSTR(html::header_cache_control), FPSTR(html::cache_control_no_cache));
    request->send(response);
    xTaskNotify(wifiManagerTask, X_CMD_REBOOT, eSetBits);
  }
  ...
  uint32_t ota_progress_millis;

  void onOTAProgress(size_t current, size_t final) {
    if (millis() - ota_progress_millis > 1000) {
      ota_progress_millis = millis();
      ESP_LOGI(TAG, "OTA Progress Current: %u bytes, Final: %u bytes", current, final);
    }
  }
  ...
  void wifiManagerLoop(void* pvParameters) {
    assert((uint32_t)pvParameters == 1);
    BaseType_t notified;
    ...
    while (1) {
      ...
      if (notified == pdPASS) {
        ...
        if (taskNotification & X_CMD_REBOOT) {
          startTimer("Reboot", 10000, reboot);
        }
      }
      if (captivePortalActive) {
